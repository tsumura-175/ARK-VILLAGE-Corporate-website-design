...
### Business Detail Pattern

- 事業内容ページの4事業は「共通見出し、メッセージ、説明文、写真スライダー、アイコン項目」の順序で統一する
- `.business-media-slider`：写真3点をフェードで切り替える共通スライダー。SPは4:3、768px以上は16:8を基準とし、768px以上では最大幅56remに抑える
- 操作部はトップページのキービジュアルと同じ「2桁番号＋赤い進行線」のページネーションを画像右下に重ね、SPのスワイプ操作も備える。画面内にある場合だけ自動再生する
- ホバー中とフォーカス中は自動再生を停止し、`prefers-reduced-motion` では自動再生とフェードを無効化する
- 素材未確定時は `sample_image_1.jpg`、`sample_image_2.jpg`、`sample_image_3.jpg` を仮配置し、支給後に同じ枠内で差し替える
- `.business-capability-grid`：対応範囲を線画アイコンと短い項目名で表示する。3項目と6項目はPCで3列、4項目はPCで2列とする
- SPではアイコン項目を1列、768px以上では2列、1024px以上では項目数に応じた列数へ展開する
- セクション内部の構造は統一しつつ、基本背景と補助背景を交互に使用してページ全体のリズムを保つ
...
- 事業内容ページは共通スライダーとアイコン一覧の反復をページ固有の視覚言語とし、背景の交互切り替えで単調さを抑える
